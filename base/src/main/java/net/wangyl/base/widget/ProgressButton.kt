package net.wangyl.base.widget
...
class ProgressImageButton(context: Context, attrs: AttributeSet?, defStyle: Int) :
    ...
    init {
        ...
        val ta = context.obtainStyledAttributes(attrs, R.styleable.base_ProgressImageButton)
        progressStrokeWidth = ta.getDimension(
            R.styleable.base_ProgressImageButton_base_pib_progress_stroke_width,
            resources.getDimension(R.dimen.base_middle_progress_width)
        )
        progressWidth = ta.getDimension(
            R.styleable.base_ProgressImageButton_base_pib_progress_width,
            resources.getDimension(R.dimen.base_middle_progress_radius) * 2
        )
        ta.recycle()

        stateListAnimator =
            AnimatorInflater.loadStateListAnimator(context, R.animator.base_button_press_alpha)
    }
...
//        typeface = FontManager.get()
        val ta = context.obtainStyledAttributes(attrs, R.styleable.base_ProgressTextButton)
        progressStrokeWidth = ta.getDimension(
            R.styleable.base_ProgressTextButton_base_ptb_progress_stroke_width,
            resources.getDimension(R.dimen.base_middle_progress_width)
        )
        progressWidth = ta.getDimension(
            R.styleable.base_ProgressTextButton_base_ptb_progress_width,
            resources.getDimension(R.dimen.base_middle_progress_radius) * 2
        )
        ta.recycle()
