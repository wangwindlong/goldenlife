...
class DefaultFormatPrinter : FormatPrinter {
    ...
    override fun printJsonResponse(
        ...
        _bodyString: String?, segments: List<String?>, message: String, responseUrl: String
    ) {
        val bodyString =
            when {
                RequestInterceptor.isJson(contentType) -> CharacterHandler.jsonFormat(_bodyString)
                RequestInterceptor.isXml(contentType) -> CharacterHandler.xmlFormat(_bodyString)
                else -> _bodyString
            }
        ...
        private fun logLines(tag: String, lines: Array<String>, withLineSize: Boolean) {
            for (line in lines) {
                ...
                for (i in 0..lineLength / maxLongSize) {
                    ...
                    Timber.d(resolveTag(tag), DEFAULT_LINE + line.substring(start, end))
                }
            }
        }
...
//            return computeKey() + tag
        ...
        private fun getResponse(
            ...
            val log: String = ((if (!TextUtils.isEmpty(segmentString)) "$segmentString - " else "") + "is success : "
