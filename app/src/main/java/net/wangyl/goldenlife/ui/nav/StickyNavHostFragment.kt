...
public open class NavHostFragment : Fragment(), NavHost {
    ...
    protected open fun createFragmentNavigator(): Navigator<out StickyNavigator.Destination> {
        return StickyNavigator(requireContext(), childFragmentManager, containerId)
    }
    ...
    public override fun onInflate(
        ...
        context.obtainStyledAttributes(attrs, R.styleable.StickyNavHostFragment).use { array ->
            val defaultHost = array.getBoolean(R.styleable.StickyNavHostFragment_defaultNavHost, false)
            if (defaultHost) {
                defaultNavHost = true
            }
        }
